...
from ProjectDirectory import ProjectDirectory

pd = ProjectDirectory()
if sys.platform == 'win32':
    libgs = windll.LoadLibrary(pd.dll_file)
elif sys.platform == 'darwin':
    try:
        libgs = cdll.LoadLibrary("libgs.9.23.dylib")
    except OSError:
    ...
    try:
        libgs = cdll.LoadLibrary("libgs.9.23.so")
    except OSError:
        # shared object file not found
        import ctypes.util
        libgs = ctypes.util.find_library('gs')
    ...
    libgs = cdll.LoadLibrary(pd.lib_file)
